...
function Print-Month ($month, $year) {
    ...
    for ($day = $firstDayOfMonth; $day -le $lastDayOfMonth; $day = $day.AddDays(1)) {
        ...
        # Highlight current day if showing current month/year and date matches today
        $isCurrentDay = ($month -eq $today.Month -and $year -eq $today.Year -and $day.Day -eq $today.Day)
        
        if ($isCurrentDay) {
            Write-Host ("{0,2}" -f $day.Day) -NoNewLine -BackgroundColor Yellow -ForegroundColor Black
        }
        else {
            Write-Host ("{0,2}" -f $day.Day) -NoNewLine 
        }
